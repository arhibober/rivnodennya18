...
class WPUF_Upload {
    ...
    public static function attach_html( $attach_id, $type = NULL ) {
        ...
        if ( wpuf_get_option( 'image_caption', 'wpuf_general', 'off' ) == 'on' ) {
            ...
            $html .= sprintf( '<textarea name="wpuf_files_data[%d][caption]" placeholder="%s">%s</textarea>', $attach_id, __( 'Заголовок', 'wpuf' ), esc_textarea( $attachment->post_excerpt ) );
            $html .= sprintf( '<textarea name="wpuf_files_data[%d][desc]" placeholder="%s">%s</textarea>', $attach_id, __( 'Опис', 'wpuf' ), esc_textarea( $attachment->post_content ) );
            $html .= '</div>';
        }

        $html .= sprintf( '<input type="hidden" name="wpuf_files[%s][]" value="%d">', $type, $attach_id );
        $html .= sprintf( '<div class="caption"><a href="#" class="btn btn-danger btn-small attachment-delete" data-attach_id="%d">%s</a></div>', $attach_id, __( 'Видалити зображення', 'wpuf' ) );
        $html .= '</li>';
