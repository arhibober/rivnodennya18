...
class WPUF_Render_Form {
    ...
    function render_form( $form_id, $post_id = NULL, $preview = false ) {
        ...
        if ( ! is_user_logged_in() && $form_settings['guest_post'] != 'true' ) {
            echo '<div class="wpuf-message">Ви не зайшли під своїм акаунтом</div>';
            //wp_login_form();
            return;
        }
        ...
        <li class="wpuf-submit">
            ...
            <?php }
				if ($_GET ["pid"] > 0)
				{
					...
					$sqlstr = "select post_name from wp_".$blog_id."_posts where id=".$_GET ["pid"];
					$name = $wpdb->get_results ($sqlstr, ARRAY_A);
					echo "<br/>
						<a href='".$path [0]["path"].$name [0]["post_name"]."'>Перейти до посту</a>";
				}
        ...
        <div class="wpuf-fields">
            <div id="wpuf-<?php echo $attr['name']; ?>-upload-container">
                <div class="wpuf-attachment-upload-filelist" data-type="file" data-required="<?php echo $attr['required']; ?>">
                    <a id="wpuf-<?php echo $attr['name']; ?>-pickfiles" data-form_id="<?php echo $form_id; ?>" class="button file-selector <?php echo ' wpuf_' . $attr['name'] . '_' . $form_id; ?>" href="#"><?php _e( 'Встановити головне зображення', 'wpuf' ); ?></a>
                    ...
                    <ul class="wpuf-attachment-list thumbnails">
                        ...
                        if ( $has_avatar ) {
                            ...
                            if ( $avatar ) {
                                echo $featured_image;
                                printf( '<br><a href="#" data-confirm="%s" class="wpuf-button button wpuf-delete-avatar">%s</a>', __( 'Ви впевнені?', 'wpuf' ), __( 'Видалити', 'wpuf' ) );
                            }
                        }
