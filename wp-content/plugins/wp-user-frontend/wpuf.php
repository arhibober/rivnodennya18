...
class WP_User_Frontend {
    ...
    function enqueue_scripts() {
        ...
        if ( isset ( $post->ID ) ) {
            ...
            <script type="text/javascript" id="wpuf-language-script">
                var error_str_obj = {
                    'required' : '<?php _e( ' - поле не може бути порожним!', 'wpuf' ); ?>',
    ...
    function plugin_scripts() {
        ...
        wp_localize_script( 'wpuf-form', 'wpuf_frontend', array(
            ...
            'error_message' => __( 'Будь ласка, заповните всі необхідні поля (з зіркочками)!', 'wpuf' ),
            'nonce'         => wp_create_nonce( 'wpuf_nonce' )
        ) );

        wp_localize_script( 'wpuf-upload', 'wpuf_frontend_upload', array(
            'confirmMsg' => __( 'Ви впевнені?', 'wpuf' ),
            'nonce'      => wp_create_nonce( 'wpuf_nonce' ),
            'ajaxurl'    => admin_url( 'admin-ajax.php' ),
            'plupload'   => array(
