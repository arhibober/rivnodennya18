		...
		if ( has_action( 'tml_request_' . $this->request_action ) ) {
			...
			switch ( $this->request_action ) {
				...
				case 'retrievepassword' :
					...
					if ( isset( $_REQUEST['error'] ) ) {
						...
						elseif ( 'expiredkey' == $_REQUEST['error'] )
							$this->errors->add( 'expiredkey', __( 'Пробачте, але даний ключ вже не актуален. Будь ласка, спробуйте знову.', 'theme-my-login' ) );
					}
				...
				default:
					...
					elseif	( $reauth )
						$this->errors->add( 'reauth', "Будь ласка, увійдіть на сайт для продовження", 'message' );
...
<?php
	 ...
	 */
	public function shortcode( $atts = '', $is_widget = false ) {
		...
		if (($is_widget) || (strstr ($_SERVER ["REQUEST_URI"], "/login/")))
			return $instance->display();
		else
			if (strstr ($_SERVER ["REQUEST_URI"], "lostpassword"))
				return $instance->display('lostpassword');
			else
				return $instance->display('profile');
	}
	...
	public static function retrieve_password() {
		...
		$message = __( 'Хтось запитав перезавантаження паролю для наступного акаунту:', 'theme-my-login' ) . "\r\n\r\n";
		$message .= network_home_url( '/' ) . "\r\n\r\n";
		$message .= sprintf( __( 'Username: %s', 'theme-my-login' ), $user_login ) . "\r\n\r\n";
		$message .= __( 'Якщо ви не розумієте, про що йде мова, просто проігноруйте цього листа.', 'theme-my-login' ) . "\r\n\r\n";
		$message .= __( 'Для зміни паролю перейдіть, будь ласка, за посиланням:', 'theme-my-login' ) . "\r\n\r\n";
