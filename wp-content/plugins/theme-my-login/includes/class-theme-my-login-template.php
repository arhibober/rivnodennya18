...
class Theme_My_Login_Template extends Theme_My_Login_Abstract {
			...
			if ($user_ID == 4)
				...
				(
					...
					(
						...
						"url"   => $path ["path"]."wp-admin"
					),
					array
					(
						"title" => "Додати новий твір",
						"url"   => $path ["path"]."wp-admin/post-new.php"
					),
					array
					(
						"title" => "Додати новий твір (швидко)",
						"url"   => $path ["path"]."dodaty-novyj-post"
					)
				);
			else
				$user_links = array
	...
	public function the_user_links() {
		...
		echo '<li><a href="' . wp_logout_url() . '">' . /*self::get_title( 'logout' )*/Вийти . '</a></li>' . "\n";
		echo '</ul>';
	}
	...
	public function the_user_avatar( $size = '' ) {
		if ( empty( $size ) )
			$size = $this->get_option( 'gravatar_size', 150 );
$size =70;
	...
	public static function get_action_template_message( $action = '' ) {
		switch ( $action ) {
			case 'register':
				...
				break;
			case 'lostpassword':
				$message = __( 'Будь-ласка, уведіть свій логін або електронну пошту, щоб отримати на пошту посилання на зміну пароля.', 'theme-my-login' );
				break;
			case 'resetpass':
