...
function the_ratings($start_tag = 'div', $custom_id = 0, $display = true) {
	...
	if(intval($postratings_ajax_style['loading']) == 1) {
		$loading = '<' . $start_tag . ' id="post-ratings-' . $ratings_id . '-loading" class="post-ratings-loading">
			<img src="' . plugins_url('wp-postratings/images/loading.gif') . '" width="16" height="16" class="post-ratings-image" />' . __( 'Дані опрацьовуються...', 'wp-postratings' ) . '</' . $start_tag . '>';
	} else {
		$loading = '';
	}
...
function process_ratings() {
	...
	if(isset($_REQUEST['action']) && $_REQUEST['action'] == 'postratings')
	{
		...
		{
			_e('Помилка голосування', 'wp-postratings');
			exit();
		}
		...
		if($rate > 0 && $post_id > 0 && check_allowtorate()) {
			...
			if(!$rated) {
				...
				printf(__('Ви вже оцінили цей пост раніше.', 'wp-postratings'), $post_id);
				exit();
			}// End if(!$rated)
...
function get_ratings_images_vote($post_id, $ratings_custom, $ratings_max, $post_rating, $ratings_image, $image_alt, $insert_half, $ratings_texts) {
	...
	if($ratings_custom) {
		for($i=1; $i <= $ratings_max; $i++) {
			...
			if($i <= $post_rating) {
				$ratings_images .= '<span id="rating_'.$post_id.'_'.$i.'" onmouseover="current_rating('.$post_id.', '.$i.', \''.$ratings_text_js.'\');" onmouseout="ratings_off('.$post_rating.', '.$insert_half.', '.$use_half_rtl.');" onclick="rate_post();" onkeypress="rate_post();" style="cursor: pointer; border: 0px;">Подобається</span>';
			} elseif($i == $insert_half) {
				if ($use_half_rtl) {
					$ratings_images .= '<span id="rating_'.$post_id.'_'.$i.'" title="'.$ratings_text.'" onmouseover="current_rating('.$post_id.', '.$i.', \''.$ratings_text_js.'\');" onmouseout="ratings_off('.$post_rating.', '.$insert_half.', '.$use_half_rtl.');" onclick="rate_post();" onkeypress="rate_post();"/>Подобається</span>';
				} else {
					$ratings_images .= '<span id="rating_'.$post_id.'_'.$i.'" title="'.$ratings_text.'" onmouseover="current_rating('.$post_id.', '.$i.', \''.$ratings_text_js.'\');" onmouseout="ratings_off('.$post_rating.', '.$insert_half.', '.$use_half_rtl.');" onclick="rate_post();" onkeypress="rate_post();"/>Подобається</span>';
				}
			} else {
